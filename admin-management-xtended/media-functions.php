...
function ame_custom_column_media_order( string $ame_column_name, int $ame_id ) {
    ...
    if( $ame_column_name == 'ame_media_order' && current_user_can( 'edit_post', $ame_id ) ) {
    	...
    	echo '<input type="text" value="' . $q_media_order->menu_order . '" size="3" maxlength="3" style="font-size:1em;" id="ame_postorder' . esc_attr(intval($ame_id)) . '" onchange="ame_ajax_order_save(' . esc_attr(intval($ame_id)) . ', \'post\');" /> <span id="ame_order_loader' . esc_attr(intval($ame_id)) . '" style="display:none;"><img src="' . AME_PLUGINFULLURL . 'img/' . AME_IMGSET . 'loader.gif" border="0" alt="" /></span>';
    	echo '</div>';
    }
}
